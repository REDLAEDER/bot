...
langs: {
    en: {
        ...
        notiBox: "Akiri BoT Connected Successfully!\Use +help for more info :>",
        approveSuccess: "Approved successfully %1 threads!",

        cantGetPendingList: "Can't get the pending list!",
        returnListPending: "»「PENDING」«❮ The whole number of threads to approve is: %1 thread ❯\\%2",
...
onReply: async function({ api, event, Reply, getLang, commandName }) {
    ...
    if (isNaN(body) && body.indexOf("c") == 0 || body.indexOf("cancel") == 0) {
        const index = (body.slice(1, body.length)).split(/\+/);
        ...
        for (const singleIndex of index) {
            if (isNaN(singleIndex) || singleIndex <= 0 || singleIndex > Reply.pending.length) return api.sendMessage(getLang("invaildNumber", singleIndex), threadID, messageID);
            api.sendMessage(getLang("notiBox"), Reply.pending[singleIndex - 1].threadID);
            count+=1;
        }
